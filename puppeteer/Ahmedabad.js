...
const mergeData = () => {
  let finaldata = [];
  const bedData = gatherAhmedabad(
    "https://vmc.gov.in/AhmedabadDistrictRural/HospitalBedsDetails.aspx?tid=1"
  );
  bedData
    .then((dt) => {})
    .catch((err) => {
  ...
  const oxyBedData = gatherAhmedabad(
    "https://vmc.gov.in/AhmedabadDistrictRural/HospitalBedsDetails.aspx?tid=4"
  );
  oxyBedData
    .then((dt) => {})
    .catch((err) => {
